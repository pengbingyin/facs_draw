...
# List to collect data for stacked plots (filename, fitc_data)
plot_data_list = []
    ...
    for filename in files:
        if filename.lower().endswith('.fcs'):
            ...
            try:
                ...
                if len(fitc_data) > 1:
                    plot_data_list.append((filename, fitc_data))
                else:
...
if plot_data_list:
    ...
    for i, (filename, fitc_data) in enumerate(plot_data_list):
        ax = axs[i]
        ...
        # Generate points (clip to 0-600)
        x = np.linspace(0, 600, 500)

        y_fitc = kde_fitc(x) * 10000

        # Add semitransparent shadow under the line
        ax.fill_between(x, 0, y_fitc, color='green', alpha=0.3)

        # Plot the line on top with half the default linewidth (default is ~1.5, so 0.75)
